...
public class ProductBuilder implements Builder {
    ...
    @Override
    public void reset() {
        this.product = new Product(0, "", new Coordinates(0, 0), "", 0F,
    ...
    public void setBPartNumber() throws ExitException {
        while (true) {
            try {
                ...
                break;
            } catch (IOException | NumberFormatException | NullPointerException | FalseValuesException |
    ...
    public void setBOrganization() throws ExitException, IOException {
        ...
        organization.setId(setBOrganizationId());
    ...
    public Integer setBOrganizationId() {
        while (true) {
            try {
                var id = (int) (Math.random() * 1000 + 1);
                validator.isIdUnique(id);
                return id;
    ...
    public String setBFullName() throws ExitException {
        while (true) {
            try {
                ...
                validator.isFullNameUnique(fullname);
                validator.emptyToNull(fullname);
                return fullname;
            } catch (IOException | NumberFormatException | FalseValuesException | UniqueException exception) {
                System.out.println(exception.getMessage());
            }
        }
    }
    ...
    public OrganizationType setBType() throws IOException, ExitException {
        while (true) {
            ...
            switch (str) {
                case "COMMERCIAL", "C" -> orgType = OrganizationType.COMMERCIAL;
                case "GOVERNMENT", "G" -> orgType = OrganizationType.GOVERNMENT;
                case "TRUST", "T" -> orgType = OrganizationType.TRUST;
                case "PRIVATE_LIMITED_COMPANY", "P" -> orgType = OrganizationType.PRIVATE_LIMITED_COMPANY;
                default -> {
                    continue;
                }
            }
