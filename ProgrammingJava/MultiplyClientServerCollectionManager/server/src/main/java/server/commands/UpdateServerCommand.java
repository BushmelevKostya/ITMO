...
public class UpdateServerCommand extends ServerCommand {
	...
	public String execute(Integer id, Product product, String login) throws SQLException {
		try {
			...
			int result = migrations.updateProduct(id, product.getName(), product.getCreationDate(), product.getPrice(), product.getPartNumber(), product.getManufactureCost(), product.getUnitOfMeasure().toString(), migrations.getUserId(login));
